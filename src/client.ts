...
export class ApiUrl {
  AnimeItem = 'https://laftel.net/api/items/v1/';
  SearchAnime = 'https://laftel.net/api/search/v1/keyword/?keyword=';
}

interface ILaftelRawAnimeInfo {
  id: number;
  name: string;
...
class LaftelAnimeInfo {
  ...
  constructor(data: ILaftelRawAnimeInfo) {
    this.id = data.id;
    this.name = data.name;
...
export async function laftelRequest(
  requestURL: string
